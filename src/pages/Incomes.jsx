...
import { Search, Plus, Trash2, Edit2, X, Filter, Loader2 } from 'lucide-react';
...
const Incomes = () => {
  ...
  const fetchIncomes = async (forceRefresh = false) => {
    try {
      ...
      if (!forceRefresh && monthCache.current[cacheKey]) {
        setIncomes(monthCache.current[cacheKey]);
        setLoading(false);
        return;
      }
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      if (activePlannedId) {
        ...
        // 2. Approve it to execute standard income log and balance change
        await api.post(`/api/v1/scheduled-transactions/${activePlannedId}/approve/`);
      } else {
        await api.post('/api/v1/income/', {
          amount: form.amount,
          wallet: form.wallet,
          income_related_id: form.category,
          description: form.description,
          income_date: form.date || new Date().toISOString().split('T')[0]
        });
      }
      ...
      alert(parseBackendError(err, 'Failed to create income entry.'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      setSubmitting(true);
      await api.put(`/api/v1/income/${editingItem.id}/`, {
        amount: form.amount,
        wallet: form.wallet,
        income_related_id: form.category,
        description: form.description,
      ...
      alert(parseBackendError(err, 'Failed to update income.'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const startEdit = (item) => {
    ...
    setForm({
      amount: item.amount,
      wallet: item.wallet,
      category: item.income_related_id,
  ...
  const filteredIncomes = incomes.filter(item => {
    if (selectedMonth) {
      const itemMonth = item.income_date?.substring(0, 7);
      if (itemMonth !== selectedMonth) return false;
    }
    if (selectedWallet && String(item.wallet) !== String(selectedWallet)) {
      return false;
    }
    if (fromDate && item.income_date < fromDate) return false;
    if (toDate && item.income_date > toDate) return false;
    if (searchQuery) {
      const text = (item.description + ' ' + (item.income_related_detail?.short_info || '')).toLowerCase();
  ...
  const getSuggestedAmounts = () => {
    if (!form.category) return [1000, 5000, 10000];
    const matches = incomes.filter(item => String(item.income_related_id) === String(form.category));
    if (matches.length === 0) return [1000, 5000, 10000];
    const freq = {};
    ...
    const fallbacks = [1000, 5000, 10000];
  ...
  const autoSelectWallet = (catId, amt, currentIncomes) => {
    if (!catId) return;
    let match = currentIncomes.find(item => 
      String(item.income_related_id) === String(catId) && 
      parseInt(item.amount) === parseInt(amt)
    );
    if (!match) {
      match = currentIncomes.find(item => String(item.income_related_id) === String(catId));
    }
  ...
  const getPieChartData = () => {
    ...
    filteredIncomes.forEach(item => {
      const cat = item.income_related_detail?.short_info || 'Other';
  ...
  return (
    <MobileLayout title="Amar Hishab">
      ...
      {/* Income Summary Header Bar */}
      <h3 style={styles.summaryTitle}>Income Summary</h3>
      ...
      <div style={styles.listContainer}>
        {loading ? (
          <p style={styles.infoText}>Loading income logs...</p>
        ) : filteredIncomes.length === 0 ? (
          <p style={styles.infoText}>No income entries found.</p>
        ) : (
          filteredIncomes.map((item) => (
            <div key={item.id} className="glass-card" style={styles.card}>
              <div style={styles.cardLeft}>
                <div style={styles.badge}>
                  {item.income_related_detail?.short_info || 'Other'}
                </div>
                ...
                <div style={styles.categoryChipsWrapper}>
                  {categories.length === 0 ? (
                    <div style={styles.noCategoryMsg}>
                      No income category has been created. To create please{' '}
                      <span 
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            autoSelectWallet(c.id, form.amount, incomes);
                          }}
                          style={{
                            ...styles.chipButton,
              ...
              >
                {submitting ? (
                  <>
                    ...
                    {editingItem ? 'Saving changes...' : 'Logging income...'}
                  </>
                ) : (
...
const styles = {
  ...
  metricValue: {
    fontSize: '22px',
    fontWeight: '700',
    color: '#34d399', // Bright emerald green highlight
